...
- **150+ commands** across Jira, Confluence, and Bitbucket — **1700+ tests**
...
| **Confluence** | 43+ | Pages, spaces, comments (inline + footer, with `resolutionStatus` open/reopened/resolved), content properties, labels, attachments, restrictions, export PDF/Word, hybrid inline-comment reanchoring (`--reanchor-map` + `--orphan-strategy=auto`: manual + LCS + footer fallback, duplicate-marker guarded + `--skip-unparseable-markers`) |
| **Bitbucket** | 30+ | PRs (+ default reviewers, comments, tasks, **`my-reviews` workspace-wide reviewer listing** with 24h cache + bounded-concurrency fan-out, `active_repos` allow-list config for deterministic scans, partial-failure warning under throttle), batch PR status (multi-repo, conflict detection), pipelines (trigger, rerun), branches, commits, repos, clone |
